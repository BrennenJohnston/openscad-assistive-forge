...
describe('birds and their perches (CW-58)', () => {
  ...
  it('★★ only puts a species on a perch it would actually use', () => {
    ...
    // A goose and a roadrunner are ground birds and nothing else.
    expect(SPECIES_PERCHES['canada goose']).toEqual(['ground'])
    expect(SPECIES_PERCHES['greater roadrunner']).toEqual(['ground'])
    // ★★ AND THE CROW IS NEVER ON THE GROUND. CW-57 measured this game's
    // greenspace at luminance under a tenth and proved a texture cannot lift
    // it, so the darkest bird on the roster would be a dark shape on a
    // near-black field. This is a measured constraint, not a taste.
    expect(SPECIES_PERCHES['american crow']).not.toContain('ground')
    expect(BIRD_SPECIES['american crow'].tierBias).toBeLessThan(0)
    // Small birds do not stand on lawns or parapets here.
  ...
  it('gives every city a roster whose species all exist', () => {
    ...
    for (const [city, roster] of Object.entries(CITY_BIRDS)) {
      ...
      for (const n of roster) expect(BIRD_SPECIES[n], `${city}/${n}`).toBeDefined()
    }
  ...
  it('picks deterministically and only from the eligible', () => {
    ...
    for (let d = 0; d < 40; d++) {
      const picked = pickBird(roster, 'ground', d)
      // Seattle's only ground bird is the pigeon.
      expect(picked).toBe('rock pigeon')
    }
    const seen = new Set()
