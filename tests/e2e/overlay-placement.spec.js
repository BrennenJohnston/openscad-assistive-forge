...
test.describe('Reference image placement (DP-5)', () => {
  ...
  test('the reference image can be handed to a design, aspect and all', async ({
    ...
    await page.locator('#overlayFileInput').setInputFiles(RASTER);
    await expect(page.locator('#overlayUseRow')).toBeVisible({
      timeout: 30000,
    });
  ...
  test('★ Use as design converts even when the picture would not start itself', async ({
    ...
    await page.evaluate(() => {
      document.querySelector(
        '#param-design_file'
      ).dataset.forgeStartConversion = '1';
    });
    await page.locator('#param-design_file').setInputFiles(RASTER);
    ...
    // And the flag is read once and cleared, so it can never leak into the
    // next file the person picks by hand.
    const left = await page.evaluate(
      () =>
        document.querySelector('#param-design_file').dataset
          .forgeStartConversion ?? null
    );
  ...
  test('placement is remembered with the project, and comes back on reload', async ({
    ...
    test.skip(
      !saved,
      'no save prompt appeared, so there is no project to save into'
    );
    await openOverlayPanel(page);
    ...
    expect(
      stored,
      'overlay-settings.json must be in the project store'
    ).not.toBeNull();
