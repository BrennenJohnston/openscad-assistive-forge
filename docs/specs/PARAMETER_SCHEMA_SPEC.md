...
This document defines the **Parameter Schema** format used by the OpenSCAD Assistive Forge. The schema is the intermediate representation between OpenSCAD Customizer annotations and the parameter UI the app builds from them.

**How it is used**: the app extracts parameters from your `.scad` file at runtime, in the browser, and converts them to this format internally to generate the form controls. Nothing writes a schema file to disk.

> **Note, 2026-08-16.** This specification used to describe a second consumer, a
> `forge extract` command that wrote `params.schema.json` files. That
> command-line tool was removed from the project on 2026-08-04 and the sections
> describing its file artifacts have been marked accordingly. The schema itself
> is unchanged and is still exactly what the app builds internally.
...
- Nested vectors (`[[1,0],[0,1]]`) are detected but rendered as raw text input. Component-by-component editing of a nested vector is still not built (re-checked 2026-08-16: `parser.js` sets `nested: true`, and `ui-generator.js` has no branch for it).
...
These names belonged to the removed command-line tool. **The app writes none of
them.** They are kept here so that anyone holding files generated by that tool
can still tell what they are.
