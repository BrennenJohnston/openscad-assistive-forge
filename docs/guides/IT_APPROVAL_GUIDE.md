...
| `style-src 'self'` | Stylesheets from same origin only. | **No `unsafe-inline`** -- CodeMirror injects its CSS in a `<style>` element, which this policy blocks; the app re-homes those rules into a constructable stylesheet, which CSP does not govern. Expect exactly one `style-src-elem` console violation, from that blocked element. This is a stricter style-src than most web apps ship with. |
