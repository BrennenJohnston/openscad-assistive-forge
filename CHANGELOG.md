...
- **Braille Charm bracelet clip shape** — `charm_shape = bracelet_clip` adds the Charm Customizer's C-clip bracelet charm (q_charm lineage, AAC prior art by Duy Do / UW WOOF3D) to the Braille Charm; it always exports standing vertically (the C profile lies on the bed, braille on the vertical outer wall) so the dots print crisply with no support fin, and its fit parameters (channel length, clip height, profile depth, wall thickness, gap width/offset, corner rounding) live under a new Bracelet Clip group shown only when the shape is selected (`@depends`)
- **Braille Charm `bed_contact_mm` parameter** — the Angled-mode bed sink (previously hardcoded at 0.6 mm) is now exposed and defaults to 1.0 mm, trimming the leaning charm's bottom edge into a flat first-layer contact strip with real surface area instead of a knife edge
...
- **Braille Charm defaults to Angled printing** — `print_orientation` now defaults to Angled (75° lean, the CHI 2024 sweet spot), so every exported STL is already oriented for optimal braille printing; the support fin defaults are slimmed for the charm's small volume (fin thickness 1.2 → 0.8 mm, bridges 4 → 2, brim 2.0 → 1.5 mm)
